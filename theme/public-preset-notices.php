...
if ( ! class_exists( 'TradePress_Public_PreSet_Notices' ) ) :
	...
	class TradePress_Public_PreSet_Notices {
		...
		public function message_list() {
			...
			$messages_array['TradePress'][0] = array(
				'type'  => 0,
				'title' => __( 'No Update Performed', 'tradepress' ),
				'info'  => __( 'We already have the latest Twitch data from your account.', 'tradepress' ),
			);

			// Login by Shortcode - search 'key' => 5 to find messages place...
			/* translators: %s: version description */
			$messages_array['login'][0] = array(
				'type'  => 1,
				'title' => __( 'Twitch.tv Reply', 'tradepress' ),
				'info'  => __( 'Twitch said: %s', 'tradepress' ),
			);
			$messages_array['login'][1] = array(
				'type'  => 1,
				'title' => __( 'Login Problem', 'tradepress' ),
				'info'  => __( 'We could not established your original page when you attempted to login. Please try again and report this problem if it continues.', 'tradepress' ),
			);
			$messages_array['login'][2] = array(
				'type'  => 1,
				'title' => __( 'Twitch Code Missing', 'tradepress' ),
				'info'  => __( 'Sorry, it appears Twitch.tv returned you without a code. Please try again and report this issue if it happens again.', 'tradepress' ),
			);

			// Streamlabs (temporary pending filter)
			$messages_array['officialstreamlabsextension'][0] = array(
				'type'  => 2,
				'title' => __( 'No Update Performed', 'tradepress' ),
				'info'  => __( 'We already have the latest Streamlabs data for you.', 'tradepress' ),
			);
