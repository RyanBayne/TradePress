...
3. ~~Economic Calendar FMP fallback provider (TradingView widget or alternative) remains undecided for alpha; document chosen fallback before beta.~~ **Closed 2026-05-02:** FMP is the sole approved provider for alpha. Fallback is None/deferred. TradingView iframe widget is a beta candidate but requires a separate embedded-widget contract if adopted; does not use Trading212, Alpaca, or Alpha Vantage which have incompatible data shapes for macro calendar events.
