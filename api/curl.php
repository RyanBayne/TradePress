 ...
 *
 * Use: $this->curl_object = new TradePress_Curl();
 *
 ...
 * @author   Ryan Bayne
 *
 * See extend-wp-http-curl.php for an alternative approach.
 */
...
if ( ! extension_loaded( 'curl' ) ) {
	trigger_error( 'cURL is not installed on your server, please install cURL to use TradePress.' );
}
if ( ! extension_loaded( 'json' ) ) {
	trigger_error( 'PECL JSON or pear JSON is not installed, please install either PECL JSON or compile pear JSON to use TradePress.' );
}

class TradePress_Curl extends WP_Http_Curl {
	...
	/**
	 * Call count option "TradePress_twitchapi_call_count" doubles
	 * as a call ID.
	 *
	 * @var mixed
	 */
	 ...
	 * becomes entry ID in class.TradePress_api_logging.php and
	...
	/**
	 * Use to tell TradePress to queue a request if the system is busy.
	 ...
	 * call within the TradePress system. Use this to learn which values are available
	 ...
	 * TradePress, WordPress and not just the Twitch API...
	 *
	 * Some parameters may need to change such as pagination cursors but most
	 * values should not change and instead use "new TradePress_Curl()"
	 *
	...
	public function parse_arguments( $optional_args ) {
		...
		$this->call_parameters = wp_parse_args(
			$optional_args,
			array(
				'type'          => $this->type,
	...
	public function get_transient() {
		...
		$append               = TradePress_encode_transient_name( $this->endpoint, $this->originating_function, $this->originating_line );
		$this->transient_name = 'TradePress_' . $prepend . '_' . $append;
	...
	public function logging_initiate() {
		if ( ! TradePress_API_Logging::ready() ) {
			return; }

		// $entryid is returned by TradePress_API_Logging and held as $loggingid in this object...
		$this->loggingid = TradePress_API_Logging::new(
			$this->call_id,
			'twitch',
			$this->type,
			86000,
	...
	public function logging_finalize( $outcome ) {
		if ( ! TradePress_API_Logging::ready() ) {
			return;}
		TradePress_API_Logging::end_logging( $this->loggingid, $outcome );
	}
	...
	public function call_execute() {
		...
		while ( $final_while == false ) {
			...
			// Add SSL verification settings for Discord API and other secure services
			if ( strpos( $this->endpoint, 'discord.com' ) !== false || strpos( $this->endpoint, 'https://' ) !== false ) {
				// Ensure proper SSL verification
				$this->curl_request['sslverify'] = true;

				// Use the default WordPress CA certificate bundle
				$this->curl_request['sslcertificates'] = ABSPATH . WPINC . '/certificates/ca-bundle.crt';

				// Set appropriate timeout for external APIs
				$this->curl_request['timeout'] = 30;

				// Add additional cURL options for better SSL handling
				if ( ! isset( $this->curl_request['curl_options'] ) ) {
					$this->curl_request['curl_options'] = array();
				}

				// Set proper SSL version - TLS 1.2 is required by Discord
				$this->curl_request['curl_options'][ CURLOPT_SSLVERSION ] = CURL_SSLVERSION_TLSv1_2;

				// Ensure proper hostname verification
				$this->curl_request['curl_options'][ CURLOPT_SSL_VERIFYHOST ] = 2;

				// Add proper connection timeout settings
				$this->curl_request['curl_options'][ CURLOPT_CONNECTTIMEOUT ] = 15;
				$this->curl_request['curl_options'][ CURLOPT_TIMEOUT ]        = 30;
			...
			TradePress_API_Logging::breakdown( $this->loggingid, $this->curl_reply );

			if ( ! isset( $this->curl_reply['response']['code'] ) ) {
				$this->logging_finalize( 'failed' );
				$final_while = true;
				return $this->logging_error( 'no response code', __( 'Curl reply does not include a response code.', 'tradepress' ) );
			}

			// Break the loop on reaching a strict limit of retries...
			if ( $calls_failed >= TradePress_RETRY_CALL_LIMIT ) {
				$this->logging_error( 'retry limit reached', __( 'Pagination process was stopped before completion due to too many failed calls.', 'tradepress' ) );
				$this->logging_finalize( 'failed' );
				break;
			}
	...
	public function set_curl_body( $body ) {
		if ( is_array( $body ) ) {
			...
			// Handle string input (like JSON or form-encoded data)
			$this->curl_request_body = $body;
		}
	}
