...
class TradePress_Alpaca_WebSocket {
	...
	public function subscribe( $symbols, $channels = array( 'trades', 'quotes', 'bars' ) ) {
		...
		$sub_msg = json_encode(
			array(
				'action'       => 'subscribe',
				'trades'       => in_array( 'trades', $channels ) ? $symbols : array(),
				'quotes'       => in_array( 'quotes', $channels ) ? $symbols : array(),
				'bars'         => in_array( 'bars', $channels ) ? $symbols : array(),
				'dailyBars'    => in_array( 'dailybars', $channels ) ? $symbols : array(),
				'statuses'     => in_array( 'statuses', $channels ) ? $symbols : array(),
				'lulds'        => in_array( 'lulds', $channels ) ? $symbols : array(),
				'cancelErrors' => in_array( 'cancel_errors', $channels ) ? $symbols : array(),
				'corrections'  => in_array( 'corrections', $channels ) ? $symbols : array(),
			)
		);
