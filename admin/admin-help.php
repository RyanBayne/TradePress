...
if ( ! class_exists( 'TradePress_Admin_Help', false ) ) :
	...
	class TradePress_Admin_Help {
		...
		public function add_tabs() {
			...
			*/
			$screen->set_help_sidebar(
				apply_filters(
					'TradePress_set_help_sidebar',
					'<p><strong>' . __( 'For more information:', 'tradepress' ) . '</strong></p>' .
					'<p><a href="https://github.com/ryanbayne/TradePress" target="_blank">' . __( 'GitHub', 'tradepress' ) . '</a></p>' . '<p><a href="https://TradePress.wordpress.com" target="_blank">' . __( 'Blog', 'tradepress' ) . '</a></p>' . '<p><a href="https://discord.gg/ScrhXPE" target="_blank">' . __( 'Discord', 'tradepress' ) . '</a></p>' . '<p><a href="https://www.patreon.com/TradePress" target="_blank">' . __( 'Patron Pledges', 'tradepress' ) . '</a></p>'
				)
			);

			$screen->add_help_tab(
				apply_filters(
					'TradePress_help_tab_support',
					array(
			...
			$screen->add_help_tab(
				apply_filters(
					'TradePress_help_tab_contribute',
					array(
				...
				$screen->add_help_tab(
					array(
						...
						'content' => '<h2>' . __( 'Developer Mode Active', 'tradepress' ) . '</h2>' .
									'<p>' . __( 'Developer mode is currently enabled. This provides additional debugging information and developer tools.', 'tradepress' ) . '</p>' . '<p><strong>' . __( 'Current Page:', 'tradepress' ) . '</strong> ' . esc_html( $screen->id ) . '</p>' . '<p><strong>' . __( 'Page Parameters:', 'tradepress' ) . '</strong> ' . esc_html( http_build_query( wp_unslash( $_GET ) ) ) . '</p>',
					)
				);
			}
		...
		public function app_status() {
			...
			if ( $cache ) {
				echo '<p>' . esc_html__( 'You are viewing cached data that is up to 120 seconds old. Refresh again soon to get the latest data.', 'tradepress' ) . '</p>';
				...
				echo '<p>' . esc_html__( 'You are viewing real-time data on this request (not cached). The data will be cached for 120 seconds.', 'tradepress' ) . '</p>';
			}
		...
		public function development() {
			...
			echo '<p>' . esc_html__(
				'Do not use these features on live sites. Feel free to test features 
        but do not feedback faults.',
				'tradepress'
			) . '</p>';
		...
		private static function get_contextual_feedback_form() {
			...
			$page      = empty( $_GET['page'] ) ? '' : sanitize_title( wp_unslash( $_GET['page'] ) );
			$tab       = empty( $_GET['tab'] ) ? '' : sanitize_title( wp_unslash( $_GET['tab'] ) );
			$configure = empty( $_GET['configure'] ) ? '' : sanitize_text_field( wp_unslash( $_GET['configure'] ) );
			...
			return TradePress_Feature_Feedback_System::render_issue_form(
				$feature_type,
				$feature_id,
				array(
