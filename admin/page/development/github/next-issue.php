...
function TRADEPRESS_GITHUB_next_issue_content( $repo_owner, $repo_name ) {
	...
	$issues = tradepress_get_github_repository_issues(
		$repo_owner,
		$repo_name,
		$github_token,
		array(
					...
					<div class="issue-meta">
						...
						<?php if ( $next_issue->comments > 0 ) : ?>
							<div class="meta-item">
								...
								<?php printf( esc_html( _n( '%d comment', '%d comments', $next_issue->comments, 'tradepress' ) ), $next_issue->comments ); ?>
							</div>
						<?php endif; ?>
					</div>
						...
						<table class="widefat next-issues-table">
							...
							<tbody>
								...
								foreach ( $issues['items'] as $issue ) :
									...
									if ( $issue->number === $next_issue->number ) {
										continue;
									}
									if ( $issue_count >= 5 ) {
										break; // Limit to 5 other issues
									}
									++$issue_count;
						...
						<?php if ( count( $issues['items'] ) > 6 ) : // 1 main issue + 5 in table ?>
							<div class="more-issues">
								...
								?>
									...
									<?php printf( esc_html__( 'View All %d Issues', 'tradepress' ), count( $issues['items'] ) ); ?>
								</a>
							</div>
						<?php endif; ?>
					<?php endif; ?>
