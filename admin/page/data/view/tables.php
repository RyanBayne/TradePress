...
function tradepress_display_record_fields( $record ) {
	...
	foreach ( $record as $field => $value ) {
		...
		// Format field name
		$field_label = ucwords( str_replace( array( '_', '-' ), ' ', $field ) );
...
function tradepress_data_tables_tab_content() {
	...
	if ( ! empty( $tables ) ) {
		...
		// Handle sorting
		$sort_by    = isset( $_GET['orderby'] ) ? sanitize_text_field( wp_unslash( $_GET['orderby'] ) ) : 'last_updated';
		$sort_order = isset( $_GET['order'] ) ? sanitize_text_field( wp_unslash( $_GET['order'] ) ) : 'desc';
		...
		usort(
			...
			function ( $a, $b ) use ( $sort_by, $sort_order ) {
				...
				switch ( $sort_by ) {
					...
					case 'last_updated':
					default:
						if ( $a['last_updated'] === 'N/A' && $b['last_updated'] === 'N/A' ) {
							return 0;
						}
						if ( $a['last_updated'] === 'N/A' ) {
							return 1;
						}
						if ( $b['last_updated'] === 'N/A' ) {
							return -1;
						}
	...
	<div class="configure-directives-container">
		<div class="directives-layout">
			...
			<div class="directives-table-container">
				<div class="tablenav top">
					<div class="alignleft actions">
						<input type="search" id="table-search-input" name="s" value="<?php echo esc_attr( isset( $_GET['s'] ) ? wp_unslash( $_GET['s'] ) : '' ); ?>" placeholder="<?php esc_attr_e( 'Search tables...', 'tradepress' ); ?>">
				...
				<?php else : ?>
					<div class="wp-list-table widefat fixed striped">
						<?php
						$current_orderby = isset( $_GET['orderby'] ) ? wp_unslash( $_GET['orderby'] ) : 'last_updated';
						$current_order   = isset( $_GET['order'] ) ? wp_unslash( $_GET['order'] ) : 'desc';
						?>
							...
							<div style="flex: 2;" class="sortable-column">
								...
								echo esc_url(
									add_query_arg(
										array(
											'orderby' => 'name',
											'order'   => ( $current_orderby === 'name' && $current_order === 'asc' ) ? 'desc' : 'asc',
										)
									)
								);
								?>
							...
							<div style="flex: 1;" class="sortable-column">
								...
								echo esc_url(
									add_query_arg(
										array(
											'orderby' => 'status',
											'order'   => ( $current_orderby === 'status' && $current_order === 'asc' ) ? 'desc' : 'asc',
										)
									)
								);
								?>
								...
								echo esc_url(
									add_query_arg(
										array(
											...
											'order'   => ( $current_orderby === 'last_updated' && $current_order === 'desc' ) ? 'asc' : 'desc',
										)
									)
								);
								?>
									...
									<div class="directive-meta">
										...
										<div>
											...
											<?php if ( $not_implemented_processes > 0 ) : ?>
												<?php echo (int) $not_implemented_processes; ?> not implemented
											<?php elseif ( $warning_processes > 0 || $error_processes > 0 ) : ?>
												<?php if ( $warning_processes > 0 ) : ?>
													<?php echo (int) $warning_processes; ?> warnings
												<?php endif; ?>
												<?php if ( $error_processes > 0 ) : ?>
													<?php echo (int) $error_processes; ?> errors
												<?php endif; ?>
											<?php else : ?>
			...
			<div class="directive-right-column">
				...
				<?php
				$manage_table        = isset( $_GET['manage'] ) ? sanitize_text_field( wp_unslash( $_GET['manage'] ) ) : '';
