...
class TradePress_Directive_Handler {
	...
	public static function test_directive( $directive_id, $symbol = null, $trading_mode = 'long', $directive_code = '' ) {
		...
		TradePress_Developer_Notices::database_notice(
			'INSERT',
			$symbol_meta_table,
			array(
				'symbol_id' => $symbol_id,
		...
		if ( ! class_exists( $directive_class ) ) {
			return array(
				'success'        => false,
				'message'        => 'Directive class not found: ' . $directive_class,
		...
		if ( $directive_id === 'rsi' ) {
			...
			$short_ma_data = $api->make_request(
				$ma_type,
				array(
					'symbol'      => $symbol,
					'interval'    => 'daily',
		...
		TradePress_Logging_Helper::log_scoring(
			'RSI Test Strategy',
			'test_' . $directive_id,
			array(
				'symbol'       => $symbol,
