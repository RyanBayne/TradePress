...
if ( ! class_exists( 'TradePress_Settings_TradingAPI' ) ) :
	...
	class TradePress_Settings_TradingAPI extends TradePress_Settings_Page {
		...
		public function update_application( $service ) {
			$key_real = filter_input( INPUT_POST, 'TradePress_api_' . $service . '_realmoney_apikey', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			...
			$key_real  = sanitize_key( wp_unslash( $key_real ) );
			$key_paper = filter_input( INPUT_POST, 'TradePress_api_' . $service . '_papermoney_apikey', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			$key_paper = sanitize_key( wp_unslash( (string) $key_paper ) );
		...
		public function is_application_being_saved() {
			...
			if ( 'POST' === strtoupper( (string) filter_input( INPUT_SERVER, 'REQUEST_METHOD', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) && in_array( $current_section, $this->api_array, true ) ) {
		...
		public function get_settings( $current_section = '' ) {
			...
			if ( 'default' == $current_section ) {
				...
				$settings = apply_filters(
					'TradePress_otherapi_switches_settings',
					array(
