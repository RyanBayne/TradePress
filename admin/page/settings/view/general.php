...
if ( ! class_exists( 'TradePress_Settings_General' ) ) :
	...
	class TradePress_Settings_General extends TradePress_Settings_Page {
		...
		public function process_favorite_tabs_form() {
			...
			$save_favorite_tabs = filter_input( INPUT_POST, 'save_favorite_tabs', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			$page               = filter_input( INPUT_GET, 'page', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			$tab                = filter_input( INPUT_GET, 'tab', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			$section            = filter_input( INPUT_GET, 'section', FILTER_SANITIZE_FULL_SPECIAL_CHARS );

			if ( empty( $save_favorite_tabs ) ||
				'tradepress-settings' !== $page ||
				'general' !== $tab ||
				'favetabs' !== $section ) {
				return;
			}
			...
			// Verify nonce
			$favorite_tabs_nonce = filter_input( INPUT_POST, 'favorite_tabs_nonce', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			if ( empty( $favorite_tabs_nonce ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $favorite_tabs_nonce ) ), 'save_tradepress_favorite_tabs' ) ) {
			...
			$favorite_tabs = isset( $_POST['tradepress_favorite_tabs'] ) ? array_map( 'sanitize_text_field', wp_unslash( (array) $_POST['tradepress_favorite_tabs'] ) ) : array();
		...
		public function save() {
			...
			switch ( $current_section ) {
				...
				case 'advanced':
					break;
				case 'systems':
					$webhooks_switch = filter_input( INPUT_POST, 'TradePress_webhooks_switch', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
					if ( ! empty( $webhooks_switch ) ) {
					...
					$gate_switch = filter_input( INPUT_POST, 'TradePress_gate_switch', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
					if ( ! empty( $gate_switch ) ) {
		...
		public function get_settings( $current_section = 'default' ) {
			...
			if ( 'default' == $current_section ) {
				...
				$settings = apply_filters(
					'TradePress_general_removal_settings',
					array(
				...
				$settings = apply_filters(
					'TradePress_general_advanced_settings',
					array(
				...
				$settings = apply_filters(
					'TradePress_general_systems_settings',
					array(
