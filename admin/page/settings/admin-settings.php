...
if ( ! class_exists( 'TradePress_Admin_Settings' ) ) :
	...
	class TradePress_Admin_Settings {
		...
		public static function save() {
			...
			if ( empty( $settings_nonce ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $settings_nonce ) ), 'TradePress-settings' ) ) {
		...
		public static function output() {
			...
			$current_tab     = empty( $current_tab ) ? self::$defaulttab : sanitize_title( wp_unslash( $current_tab ) );
			$current_section = filter_input( INPUT_GET, 'section', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			$current_section = empty( $current_section ) ? 'default' : sanitize_title( wp_unslash( $current_section ) );
			...
			$tradepress_error = filter_input( INPUT_GET, 'TradePress_error', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			if ( ! empty( $tradepress_error ) ) {
				self::add_error( stripslashes( sanitize_text_field( wp_unslash( $tradepress_error ) ) ) );
			}

			$tradepress_message = filter_input( INPUT_GET, 'TradePress_message', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			if ( ! empty( $tradepress_message ) ) {
				self::add_message( stripslashes( sanitize_text_field( wp_unslash( $tradepress_message ) ) ) );
			}

			$tradepress_info = filter_input( INPUT_GET, 'TradePress_info', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
			if ( ! empty( $tradepress_info ) ) {
				self::add_info( stripslashes( sanitize_text_field( wp_unslash( $tradepress_info ) ) ) );
			}
