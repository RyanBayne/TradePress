...
class TradePress_Admin_Automation {
	...
	public function ajax_stop_algorithm() {
		...
		// Log the event
		$this->logger->info(
			'Algorithm stopped manually',
			TradePress_Logger::CAT_ALGORITHM,
			array(
				'runtime' => $runtime,
			)
		);
	...
	private function simulate_algorithm_activity() {
		...
		// Simulate an error
		$this->logger->error(
			'Failed to retrieve data for GOOG',
			TradePress_Logger::CAT_API,
			array(
				'error_code'    => 429,
	...
	private function get_diagnostic_data() {
		// Get log entries
		$logs = $this->logger->get_log_entries(
			array(
				'number'  => 20,
				'orderby' => 'timestamp',
				'order'   => 'DESC',
