						...
						<div class="component-metrics">
							...
							<div class="metric-item">
								...
								<span class="metric-value">
									<?php
									$health_score = 0;
									if ( $data_import_status === 'running' ) {
										$health_score += 50;
									}
									if ( $scoring_status === 'running' ) {
										$health_score += 50;
									}
					...
					<div class="freshness-item">
						...
						<span class="freshness-value">
							...
							if ( $market_status_last_update ) {
								// Escape date and human_time_diff output for safe HTML rendering
								echo esc_html( wp_date( 'Y-m-d H:i:s', $market_status_last_update ) );
					...
					<div class="cron-global-actions">
						...
						echo esc_url(
							wp_nonce_url(
								add_query_arg(
									array(
										'action' => 'clear_all_tradepress_crons',
										'page'   => 'tradepress_automation',
										'tab'    => 'cron',
									),
									admin_url( 'admin.php' )
								),
								'tradepress_clear_cron'
							)
						);
						?>
									" class="button clear-cron-button" onclick="return confirm('<?php esc_attr_e( 'Are you sure you want to clear all scheduled TradePress CRON jobs? This cannot be undone.', 'tradepress' ); ?>');">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td>
							<code>
							<?php
							// Escape URL output for safe HTML rendering
							echo esc_url( site_url( 'wp-cron.php?doing_wp_cron' ) );
							?>
							</code>
							<p class="description">
