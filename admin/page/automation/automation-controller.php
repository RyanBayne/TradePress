...
class TradePress_Admin_Automation_Controller {
		...
		if ( $result ) {
			wp_send_json_success(
				array(
					...
					'message'    => sprintf(
						esc_html__( '%1$s %2$s successfully', 'tradepress' ),
						ucfirst( $component ),
			...
			wp_send_json_error(
				array(
					...
					'message' => sprintf(
						esc_html__( 'Failed to %1$s %2$s', 'tradepress' ),
						$action,
						$component
					),
				)
		...
		if ( $success ) {
			wp_send_json_success(
				array(
					...
					'message'    => sprintf(
						esc_html__( 'All background processes %s successfully', 'tradepress' ),
						( $action === 'start' ? 'started' : 'stopped' )
					),
					'is_running' => ( $action === 'start' ),
					'components' => $results,
	...
	private function can_access_development_automation() {
		...
		return function_exists( 'tradepress_can_access_development_views' ) ? tradepress_can_access_development_views() : false;
	}
