...
function tradepress_register_symbol_tracking() {
	...
	$symbol = filter_input( INPUT_GET, 'symbol', FILTER_SANITIZE_FULL_SPECIAL_CHARS );

	if ( ! empty( $symbol ) ) {
		tradepress_track_symbol_view( sanitize_text_field( wp_unslash( $symbol ) ) );
	}
...
function tradepress_normalize_earnings_records_for_storage( $records ) {
	...
	foreach ( $records as $record ) {
		...
		$normalized_records[] = array_merge(
			$record,
			array(
...
function tradepress_display_cron_notices() {
	...
	$cron_run   = filter_input( INPUT_GET, 'cron_run', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	$cron_job   = filter_input( INPUT_GET, 'job', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	$cron_clear = filter_input( INPUT_GET, 'cron_clear', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	if ( $cron_run === 'success' && ! empty( $cron_job ) ) {
		$job_name  = sanitize_text_field( wp_unslash( $cron_job ) );
