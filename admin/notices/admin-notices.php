	...
	class TradePress_Admin_Notices {
		...
		public static function hide_notices() {
			if ( isset( $_GET['TradePress-hide-notice'] ) && isset( $_GET['_TradePress_notice_nonce'] ) ) {
				...
				$hide_notice = sanitize_text_field( wp_unslash( $_GET['TradePress-hide-notice'] ) );
		...
		public static function error( $title, $desc, $dismissible = false ) {
			...
			if ( $dismissible ) {
				$d = ' is-dismissible'; }
			?><div class="notice notice-error<?php echo esc_attr( $d ); ?>"><p><?php echo '<strong>' . esc_html( $title ) . ': </strong>' . wp_kses_post( $desc ); // $desc may contain pre-sanitized HTML from wp_kses_post ?></p></div>
			<?php
		}
		...
		public static function warning( $title, $desc, $dismissible = false ) {
			...
			if ( $dismissible ) {
				$d = ' is-dismissible'; }
			?>
		<div class="notice notice-warning<?php echo esc_attr( $d ); ?>"><p><?php echo '<strong>' . esc_html( $title ) . ': </strong>' . wp_kses_post( $desc ); ?></p></div>
			<?php
		}
		...
		public static function success( $title, $desc, $dismissible = false ) {
			...
			if ( $dismissible ) {
				$d = ' is-dismissible'; }
			?>
		<div class="notice notice-success<?php echo esc_attr( $d ); ?>"><p><?php echo '<strong>' . esc_html( $title ) . ': </strong>' . wp_kses_post( $desc ); ?></p></div>
			<?php
		}
		...
		public static function info( $title, $desc, $dismissible = false ) {
			...
			if ( $dismissible ) {
				$d = ' is-dismissible'; }
			?>
		<div class="notice notice-info<?php echo esc_attr( $d ); ?>"><p><?php echo '<strong>' . esc_html( $title ) . ': </strong>' . wp_kses_post( $desc ); ?></p></div>
			<?php
		}
		...
		public static function demo_mode_indicator( $demo_title = '', $demo_message = '', $live_title = '', $live_message = '', $position = 'default' ) {
			...
			if ( $is_demo_mode ) :
				?>
			<!-- Demo Indicator -->
			<div class="demo-indicator <?php echo esc_attr( $position_class ); ?>" 
				<?php
				if ( $container_style ) {
			...
			<?php else : ?>
			<!-- Live Mode Indicator -->
			<div class="live-indicator <?php echo esc_attr( $position_class ); ?>" 
				<?php
				if ( $container_style ) {
					echo 'style="' . esc_attr( $container_style ) . '"';}
				?>
