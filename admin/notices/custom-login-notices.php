 ...
 * @package  TradePress Login Extension
 * @since    1.0.0
 */
 
if ( ! defined( 'ABSPATH' ) ) { exit; }

if( !class_exists( 'TradePress_Custom_Login_Messages') ) :
...
class TradePress_Custom_Login_Messages {
    ...
    */
    public function add_error( $message ) {     
        $this->TradePress_login_messages[] = array( 
            'type' => 'error', 
            'message' => $message 
        );
        
        update_option( 'TradePress_login_messages', $this->TradePress_login_messages );
    }
    ...
    */
    public function build_messages( $message ) {
        ...
        if( $this->TradePress_login_messages ) {                
            foreach( $this->TradePress_login_messages as $key => $error ) {
                if( $error['type'] == 'info' ) {
