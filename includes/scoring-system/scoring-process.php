...
class TradePress_Scoring_Process extends TradePress_Background_Processing {
	...
	private function calculate_symbol_scores( $item ) {
		...
		try {
			...
			$this->log_process_activity(
				'info',
				'Starting symbol score calculation',
				array(
			...
			foreach ( $symbols as $symbol_code ) {
				try {
					...
					$this->log_process_activity(
						'error',
						"Exception processing symbol {$symbol_code}",
						array(
			...
			$this->log_process_activity(
				'error',
				'Exception in score calculation',
				array(
					'error' => $e->getMessage(),
					'retry' => $retry_count,
				)
	...
	private function store_symbol_score( $symbol_code, $score ) {
		update_option(
			"tradepress_symbol_score_{$symbol_code}",
			array(
				'score'         => $score,
				'timestamp'     => current_time( 'timestamp' ),
	...
	private function process_symbol_post( $item ) {
		...
		try {
			...
			if ( class_exists( 'TradePress_Scoring_Algorithm' ) ) {
				...
				$this->log_process_activity(
					'info',
					"Calculated score for {$symbol->post_title}: {$score['total_score']}",
					array(
						'symbol_id'     => $symbol_id,
						'score_details' => $score,
					)
	...
	public function start_wordpress_process( $symbols = array() ) {
		$this->log_process_activity(
			'info',
			'Starting WordPress symbol post processing',
			array(
				'symbol_count' => count( $symbols ),
			)
		);
