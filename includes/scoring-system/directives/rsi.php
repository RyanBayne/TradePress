...
class TradePress_Scoring_Directive_RSI extends TradePress_Scoring_Directive_Base {
	...
	public function calculate_score( $symbol_data, $config = array() ) {
		...
		TradePress_Developer_Flow_Logger::log_action(
			'CONFIG_LOADED',
	...
	private function get_rsi_data( $symbol, $config = array() ) {
		...
		if ( $cached_result !== false ) {
			TradePress_Developer_Flow_Logger::log_cache(
				'CHECK',
				'rsi_technical_indicators',
				'HIT',
				array(
	...
	public function fetch_fresh_rsi_data( $symbol, $params ) {
		...
		if ( is_wp_error( $api ) ) {
			TradePress_Developer_Flow_Logger::log_api(
				'factory',
				'create_error',
				'FAILED',
				array(
		...
		TradePress_Developer_Flow_Logger::log_action(
			'API_CALL_ALPHAVANTAGE',
			'Using Alpha Vantage RSI endpoint',
			array(
				'symbol'      => $symbol,
		...
		TradePress_Developer_Flow_Logger::log_action(
			'USAGE_TRACKED',
			'API call tracked for rate limiting',
			array(
				'provider' => $provider_id,
				'endpoint' => 'RSI',
				'success'  => true,
