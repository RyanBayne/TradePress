...
class TradePress_Earnings_Whisper_Scorer {
	...
	private function calculate_whisper_vs_consensus_score( $whisper, $consensus ) {
		...
		if ( $difference_pct > 5 ) {
			return 90;      // Strong positive divergence
		}
		if ( $difference_pct > 2 ) {
			return 75;      // Moderate positive divergence
		}
		if ( $difference_pct > 0 ) {
			return 60;      // Slight positive divergence
		}
		if ( $difference_pct > -2 ) {
			return 50;     // Neutral
		}
		if ( $difference_pct > -5 ) {
			return 40;     // Slight negative divergence
		}
	...
	private function calculate_guidance_score( $guidance, $consensus ) {
		...
		if ( $guidance_val > $consensus_val ) {
			return 75; // Guidance above consensus
		}
		if ( $guidance_val < $consensus_val ) {
			return 35; // Guidance below consensus
		}
	...
	private function calculate_beat_miss_score( $beat_miss_pct ) {
		...
		if ( $percentage > 10 ) {
			return 90;         // Large beat
		}
		if ( $percentage > 5 ) {
			return 75;          // Moderate beat
		}
		if ( $percentage > 0 ) {
			return 60;          // Small beat
		}
		if ( $percentage > -5 ) {
			return 40;         // Small miss
		}
	...
	private function calculate_revenue_growth_score( $growth_text ) {
		...
		if ( ! empty( $matches[1] ) ) {
			...
			if ( $growth_pct > 10 ) {
				return 75;     // Good growth
			}
			if ( $growth_pct > 5 ) {
				return 60;      // Moderate growth
			}
			if ( $growth_pct > 0 ) {
				return 55;      // Slight growth
			}
			if ( $growth_pct > -5 ) {
				return 45;     // Slight decline
			}
			if ( $growth_pct > -10 ) {
				return 30;    // Moderate decline
			}
	...
	private function extract_numeric_value( $text ) {
		...
		preg_match( '/[\-\+]?\d+\.?\d*/', str_replace( array( '$', '€', '£' ), '', $text ), $matches );
