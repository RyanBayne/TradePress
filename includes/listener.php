...
if ( ! class_exists( 'TradePress_Listener' ) ) :
	...
	class TradePress_Listener {
		...
		private function process_post_requests() {
			...
			if ( ! isset( $_POST['TradePress_form_action'] ) ) {
				...
				if ( isset( $_POST['current_task_nonce'] ) && isset( $_POST['set_current_task'] ) ) {
					...
					return;
				} elseif ( isset( $_POST['task_notes_nonce'] ) && isset( $_POST['update_working_notes'] ) ) {
					$this->process_current_task_forms( 'update_task_working_notes' );
					return;
				} elseif ( isset( $_POST['subtasks_nonce'] ) && isset( $_POST['update_subtasks'] ) ) {
					$this->process_current_task_forms( 'update_subtasks' );
					return;
				} elseif ( isset( $_POST['task_status_nonce'] ) && ( isset( $_POST['mark_in_progress'] ) || isset( $_POST['mark_completed'] ) ) ) {
			...
			if ( empty( $wpnonce ) || ! wp_verify_nonce( $wpnonce, $action ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'tradepress' ) );
			}
		...
		private function process_current_task_forms( $action ) {
			...
			// Verify the nonce
			$nonce_val = isset( $_POST[ $nonce_field ] ) ? sanitize_text_field( wp_unslash( $_POST[ $nonce_field ] ) ) : '';
			if ( empty( $nonce_val ) || ! wp_verify_nonce( $nonce_val, $nonce_action ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'tradepress' ) );
			}
		...
		private function process_admin_actions() {
			...
			$req_nonce = isset( $_REQUEST['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ) : '';
			if ( empty( $req_nonce ) || ! wp_verify_nonce( $req_nonce, $action ) ) {
				wp_die( esc_html__( 'Security check failed. Please try again.', 'tradepress' ) );
			}
