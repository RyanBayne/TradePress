...
final class WordPressTradePress {
	...
	private function define_hard_constants() {
		...
		if ( ! defined( 'TRADEPRESS_MIN_WP_VERSION' ) ) {
			define( 'TRADEPRESS_MIN_WP_VERSION', $this->min_wp_version ); }
		if ( ! defined( 'TRADEPRESS_UPLOADS_DIR' ) ) {
			define( 'TRADEPRESS_UPLOADS_DIR', $upload_dir['basedir'] . 'tradepress-uploads/' ); }
		if ( ! defined( 'TRADEPRESS_WORDPRESSORG_SLUG' ) ) {
			define( 'TRADEPRESS_WORDPRESSORG_SLUG', false ); }

		// Support (project) constants.
		if ( ! defined( 'TRADEPRESS_HOME' ) ) {
			define( 'TRADEPRESS_HOME', 'https://TradePress.wordpress.com' ); }
		if ( ! defined( 'TRADEPRESS_FORUM' ) ) {
			define( 'TRADEPRESS_FORUM', 'https://wordpress.org/support/plugin/TradePress' ); }
		if ( ! defined( 'TRADEPRESS_TWITTER' ) ) {
			define( 'TRADEPRESS_TWITTER', 'https://twitter.com/TradePress' ); }
		if ( ! defined( 'TRADEPRESS_DONATE' ) ) {
			define( 'TRADEPRESS_DONATE', 'https://www.patreon.com/TradePress' ); }
		if ( ! defined( 'TRADEPRESS_GITHUB' ) ) {
			define( 'TRADEPRESS_GITHUB', 'https://github.com/ryanbayne/TradePress' ); }
		if ( ! defined( 'TRADEPRESS_DISCORD' ) ) {
			define( 'TRADEPRESS_DISCORD', 'https://discord.gg/ScrhXPE' ); }

		// Author (social) constants - can act as default when support constants are false.
		if ( ! defined( 'TRADEPRESS_AUTHOR_HOME' ) ) {
			define( 'TRADEPRESS_AUTHOR_HOME', 'https://ryanbayne.wordpress.com' ); }
		if ( ! defined( 'TRADEPRESS_AUTHOR_TWITTER' ) ) {
			define( 'TRADEPRESS_AUTHOR_TWITTER', 'http://www.twitter.com/ryan_r_bayne' ); }
		if ( ! defined( 'TRADEPRESS_AUTHOR_FACEBOOK' ) ) {
			define( 'TRADEPRESS_AUTHOR_FACEBOOK', 'https://www.facebook.com/ryanrbayne' ); }
		if ( ! defined( 'TRADEPRESS_AUTHOR_DONATE' ) ) {
			define( 'TRADEPRESS_AUTHOR_DONATE', 'https://www.patreon.com/TradePress' ); }
		if ( ! defined( 'TRADEPRESS_AUTHOR_GITHUB' ) ) {
			define( 'TRADEPRESS_AUTHOR_GITHUB', 'https://github.com/RyanBayne' ); }
		if ( ! defined( 'TRADEPRESS_AUTHOR_LINKEDIN' ) ) {
			define( 'TRADEPRESS_AUTHOR_LINKEDIN', 'https://www.linkedin.com/in/ryanrbayne/' ); }
		if ( ! defined( 'TRADEPRESS_AUTHOR_DISCORD' ) ) {
			define( 'TRADEPRESS_AUTHOR_DISCORD', 'https://discord.gg/ScrhXPE' ); }

		// Constants for the TradePress Community Network API
		if ( ! defined( 'TRADEPRESS_KEY_NAME' ) ) {
			define( 'TRADEPRESS_KEY_NAME', 'name' );}
		if ( ! defined( 'TRADEPRESS_DEFAULT_TIMEOUT' ) ) {
			define( 'TRADEPRESS_DEFAULT_TIMEOUT', 5 );}
		if ( ! defined( 'TRADEPRESS_DEFAULT_RETURN_TIMEOUT' ) ) {
			define( 'TRADEPRESS_DEFAULT_RETURN_TIMEOUT', 20 );}
		if ( ! defined( 'TRADEPRESS_TOKEN_SEND_METHOD' ) ) {
			define( 'TRADEPRESS_TOKEN_SEND_METHOD', 'HEADER' );}
		if ( ! defined( 'TRADEPRESS_RETRY_CALL_LIMIT' ) ) {
			define( 'TRADEPRESS_RETRY_CALL_LIMIT', 5 );}
		if ( ! defined( 'TRADEPRESS_CERT_PATH' ) ) {
			define( 'TRADEPRESS_CERT_PATH', '' );}
		if ( ! defined( 'TRADEPRESS_CALL_LIMIT_DEFAULT' ) ) {
			define( 'TRADEPRESS_CALL_LIMIT_DEFAULT', '15' );}
		if ( ! defined( 'TRADEPRESS_CALL_LIMIT_DOUBLE' ) ) {
			define( 'TRADEPRESS_CALL_LIMIT_DOUBLE', '30' );}
		if ( ! defined( 'TRADEPRESS_CALL_LIMIT_MAX' ) ) {
			define( 'TRADEPRESS_CALL_LIMIT_MAX', '60' );}
		if ( ! defined( 'TRADEPRESS_CALL_LIMIT_SETTING' ) ) {
			define( 'TRADEPRESS_CALL_LIMIT_SETTING', TRADEPRESS_CALL_LIMIT_MAX );}
	}
	...
	private function define_wp_reliant_constants() {
		if ( ! defined( 'TRADEPRESS_CURRENTUSERID' ) ) {
			define( 'TRADEPRESS_CURRENTUSERID', get_current_user_id() );}
		if ( ! defined( 'TRADEPRESS_BETA' ) ) {
			define( 'TRADEPRESS_BETA', get_option( 'TradePress_beta_testing', 0 ) ); }
	}
	...
	public function ajax_get_api_calls() {
		...
		$all_calls = array();
		if ( $current_register ) {
			$all_calls = array_merge( $all_calls, $current_register );
		}
		if ( $previous_register ) {
			$all_calls = array_merge( $all_calls, $previous_register );
		}
	...
	public function enqueue_admin_scripts() {
		...
		if ( is_admin() && is_string( $page ) && strpos( $page, 'tradepress' ) !== false ) {
	...
	public function load_systems() {
		...
		if ( get_option( 'TradePress_webhooks_switch' ) == 'yes' ) {
			...
			if ( ! defined( 'TradePress_WEBHOOK_CALLBACK' ) ) {
				define( 'TradePress_WEBHOOK_CALLBACK', $url ); }
			unset( $url );
	...
	public function output_actions() {
		if ( 'yes' !== get_option( 'TradePress_display_actions' ) ) {
			return; }
	...
	public function output_filters() {
		if ( 'yes' !== get_option( 'TradePress_display_filters' ) ) {
			return; }
