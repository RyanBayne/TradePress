...
class TradePress_Test_Runner {
    ...
    private $tabs = [
        ...
        'trading212' => [
            ...
            'description' => 'Live endpoint tests for the Trading212 integration  runs against your configured API key',
            'tab_ref' => 'TAB08'
        ],
        'directives' => [
    ...
    private function render_directives_tab() {
        ...
        .tp-dir-badge.unknown, .tp-dir-badge.pending { background: #f0f0f1; color: #666; }
        .tp-dir-badge.error { background: #fee2e2; color: #991b1b; }
        .tp-dir-chevron { font-size: 10px; color: #888; display: inline-block; transition: transform 0.18s; }
        ...
        .tp-dir-math-head { background: #eef2f7; padding: 5px 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; color: #444; border-bottom: 1px solid #dde3ea; }
        ...
        (function($) {
            ...
            function badge(status) { return '<span class="tp-dir-badge ' + esc(status) + '">' + esc(status) + '</span>'; }
            ...
            var stepMeta = {
                ...
                scoring_runs:  { label: 'Scoring Executes',  desc: function(r) { var d = r.detail || {}; return d.score_value !== null ? 'Returned: <strong>' + esc(String(d.score_value)) + '</strong>' : 'Returned a valid type'; } }
            };
            ...
            function renderDetailPanel(r) {
                ...
                $.each(tests, function(key, passed) {
                    ...
                    stepsHtml += '<div class="step-body"><div class="step-label">' + esc(meta.label) + '</div>';
                ...
                var mathHtml = '';
                if (detail.score_value !== null) {
                    var pct = detail.score_percent || 0;
                    var barColor = pct >= 75 ? '#16a34a' : pct >= 50 ? '#2271b1' : pct >= 25 ? '#b45309' : '#dc2626';
                    mathHtml += '<div class="tp-dir-math-box"><div class="tp-dir-math-head">Score Calculation</div><div class="tp-dir-math-inner">';
                    mathHtml += '<div class="tp-dir-math-input"><div class="tp-dir-math-sub">Dummy Input <em>(NVDA)</em></div><table class="tp-dir-input-tbl">';
                    ...
                    if (di.bollinger_lower !== undefined) { mathHtml += '<tr><td>Bollinger Band</td><td>' + esc(di.bollinger_lower) + ' / <strong>' + esc(di.bollinger_middle) + '</strong> / ' + esc(di.bollinger_upper) + '</td></tr>'; }
                    ...
                    mathHtml += '</table></div>';
                    mathHtml += '<div class="tp-dir-math-output"><div class="tp-dir-math-sub">Score Result</div>';
                    mathHtml += '<div class="tp-dir-score-display"><span class="score-num" style="color:' + barColor + '">' + esc(detail.score_value) + '</span><span class="score-sep"> / </span><span class="score-max">' + esc(detail.max_score_value) + '</span></div>';
                    mathHtml += '<div class="score-pct">' + esc(pct) + '%</div><div class="tp-dir-score-bar"><div class="tp-dir-score-bar-fill" style="width:' + Math.min(pct, 100) + '%;background:' + barColor + '"></div></div></div>';
                    mathHtml += '</div></div>';
                }

                var issuesHtml = '';
                if (issues.length) {
                    issuesHtml += '<div class="tp-dir-panel-issues"><strong style="color:#991b1b">&#9888; Issues</strong><ul>';
    ...
    private function render_trading212_tab() {
        ...
        if ( 'live' === $environment ) {
            echo ' &mdash; <span style="color:#d63638;">' . esc_html__( 'Live environment  read-only tests only, no orders will be placed.', 'tradepress' ) . '</span>';
        }
        echo '</div>';
        ...
        // Inline JS  scoped to this tab, no separate file needed.
        ?>
        <style>
        ...
        try {
            ...
            switch ( $test_suite ) {
                ...
                case 'directive_single':
                    ...
                    // Files use hyphens; directive IDs use underscores  try both.
