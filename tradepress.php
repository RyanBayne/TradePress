...
if ( ! defined( 'TRADEPRESS_VERSION' ) ) {
	define( 'TRADEPRESS_VERSION', '1.0.95' ); }
if ( ! defined( 'TRADEPRESS_PLUGIN_DIR_PATH' ) ) {
	define( 'TRADEPRESS_PLUGIN_DIR_PATH', plugin_dir_path( __FILE__ ) ); }
if ( ! defined( 'TRADEPRESS_PLUGIN_DIR' ) ) {
	define( 'TRADEPRESS_PLUGIN_DIR', plugin_dir_path( __FILE__ ) ); }
if ( ! defined( 'TRADEPRESS_PLUGIN_URL' ) ) {
	define( 'TRADEPRESS_PLUGIN_URL', plugin_dir_url( __FILE__ ) ); } // http://domain/tradepress/wp-content/plugins/TradePress/
if ( ! defined( 'TRADEPRESS_PLUGIN_FILE' ) ) {
	define( 'TRADEPRESS_PLUGIN_FILE', TRADEPRESS_PLUGIN_DIR ); }
if ( ! defined( 'TRADEPRESS_PLUGIN_BASENAME' ) ) {
	define( 'TRADEPRESS_PLUGIN_BASENAME', plugin_basename( __FILE__ ) ); }// TradePress/tradepress.php
if ( ! defined( 'TRADEPRESS_MAINFILE' ) ) {
	define( 'TRADEPRESS_MAINFILE', __FILE__ ); }
if ( ! defined( 'TRADEPRESS_TESTING' ) ) {
	define( 'TRADEPRESS_TESTING', getenv( 'TRADEPRESS_TESTING' ) === 'true' ); }
...
function tradepress_normalize_admin_request_vars() {
	...
	$post_type_get  = filter_input( INPUT_GET, 'post_type', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
	$post_type_post = filter_input( INPUT_POST, 'post_type', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
	$post_types     = is_array( $post_type_post ) ? $post_type_post : $post_type_get;
...
if ( ! class_exists( 'WordPressTradePress' ) ) :
	...
	if ( ! defined( 'TRADEPRESS_REQUEST_KEY' ) ) {
		define( 'TRADEPRESS_REQUEST_KEY', $_SERVER['REQUEST_TIME_FLOAT'] . rand( 10000, 99999 ) ); }
